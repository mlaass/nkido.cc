...
description: Designing stereo support that's both type-safe and invisible — you write mono code, it auto-lifts to stereo where it has to.
date: 2026-04-22
author: mlaass
category: post
...
A mono signal is a single `buffer` index. A stereo signal is two buffer indices — `buffer` is the left channel, `right_buffer` is the right — allocated adjacent to each other. The `channels` field isn't checked at runtime. It's a type, and the compiler tracks it through every expression.
...
- **Adjacent buffers**: because the compiler allocates `out_left` and `out_right` adjacent, "shift by 1" gets the right channel. Same for inputs. This keeps the dispatch path short — no extra lookups.
- **State IDs XOR'd for the R pass**: the state pool keyed each channel independently. Left channel's state ID is the normal path hash; right channel's is `state_id ^ 0x9E3779B9` (the golden-ratio fractional-bit constant, used just because it scatters bits well). This means the two channels have separate biquad state, separate delay buffers — exactly what you'd expect from a stereo filter — without the compiler having to emit two instructions.
...
Writing stereo Akkado feels identical to writing mono Akkado, until you explicitly go stereo — at which point the rest of the chain follows without modification. The compiler is the one source of truth about which buffers are stereo, which simplifies the VM (one generic opcode dispatch, not N stereo-aware variants) and gives honest error messages when the types don't line up.
...
See also: [Mono & Stereo Signals — reference](/docs/reference/builtins/stereo) for the builtins this post dances around.
