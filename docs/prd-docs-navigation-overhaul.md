...
The exact `subgroup` labels above are illustrative — the canonical list is whatever the manifest currently emits (see §5.2). The sidebar **renders whatever sub-groups appear in the manifest**; it doesn't hard-code them.
...
1. **`src/lib/data/docs-manifest.json`** — already generated by the upstream-mirror pipeline; carries `title`, `slug`, `url`, `group`, `subgroup`, `order`, `keywords`, `headings` per entry. Used directly for Reference.
...
Concepts and Tutorials sidebar entries come from `manifest.entries['concepts']` and `manifest.entries['tutorials']` — both flat lists ordered by frontmatter `order`. (The build pipeline already populates these.)
...
type SidebarTree = {
  ...
  reference: {
    ...
    language:      SidebarLeaf[];                // flat (no subgroup metadata)
    'mini-notation': SidebarLeaf[];              // flat (no subgroup metadata)
  };
};
...
For `reference/builtins`: bucket entries by `entry.subgroup`. Order of buckets comes from `docs-sidebar-overrides.ts`; missing-from-override sub-groups go to the end alphabetically with a one-line warning during build (`tolerant build` per v2 PRD pattern). Within a bucket, entries are ordered by `entry.order`.

`reference/language` and `reference/mini-notation`: don't have `subgroup` metadata in the manifest today. Render flat, ordered by `entry.order`. Future expansion (out of scope) could add `subgroup` fields to those frontmatters too.
...
The existing `docs-manifest.json` already includes a `headings: string[]` field per entry (extracted from the markdown body). For pages built with `+page.md`, this is populated. For `.svelte` content pages (e.g. concept/tutorial pages that author HTML directly, like `/docs/concepts/+page.svelte`), there's no heading list in the manifest — these are hub pages and don't need a TOC.
...
The only schema additions are to **concept** and **tutorial** frontmatter (upstream `nkido` if those docs are mirrored, or website-local if authored here):
...
category: tutorial            # existing
order: 1                      # REQUIRED, used to order the sidebar
keywords: [...]               # existing
---
```

**Audit & migration**: today, some concept/tutorial pages have `order:` and some don't (a quick scan of `/docs/concepts/*/+page.md` and `/docs/tutorials/*/+page.md` is part of Phase 0). Pages missing `order:` are assigned by the build script in alphabetical-slug order with a build warning so they sort deterministically until a human assigns one.
...
| `src/lib/components/Docs/SearchBox.svelte` | Stays | Component unchanged; just rendered in a new location (sidebar header). The existing `.trigger` button styling is reused with a width override. |
...
| `src/lib/data/docs-manifest.json` | Stays | Build artifact, unchanged shape. (Already has needed fields.) |
| `src/lib/data/docs-sidebar.json` | **New** | Build artifact emitted by `build-sidebar.ts`. |
...
| Upstream `nkido` concept/tutorial frontmatter | **Modified (audit pass)** | Add `order:` field where missing. |
...
| `src/routes/docs/concepts/+page.svelte` | Add Thinking in NKIDO to the card grid (same — auto-included). |
...
| (Upstream `nkido` repo) `nkido/web/static/docs/concepts/*.md` & `tutorials/*.md` frontmatter | Add `order:` field where missing. |
...
**Goal:** Land the build-sidebar step and the override file. Site behavior unchanged.

- Create `scripts/build-sidebar.ts`. Read `docs-manifest.json` + `docs-sidebar-overrides.ts` (the latter starts as a stub). Emit `src/lib/data/docs-sidebar.json`.
- Wire into `package.json` prebuild (after manifest build, before `vite build`).
- Add `docs-sidebar.json` to `.gitignore` if it isn't already (it's a generated artifact).
- Audit upstream concept/tutorial frontmatter for missing `order:` — add via PR(s) against the `nkido` repo. Hand-roll the override file for reference top-level + sub-group ordering.

**Verification:** `bun run build` succeeds; `docs-sidebar.json` is emitted with the expected shape; warnings logged for any missing frontmatter.
...
- Wire Pagefind `<SearchBox>` into the sidebar header. Remove from old top-tab bar location.
...
- `docs-sidebar.json` is generated and contains:
  ...
  - `reference.language` and `reference.mini-notation` flat lists.
...
- `DocsTOC.svelte` heading-extraction logic (if extracted into a helper): given a list of `{ depth, text }`, produces the right nested structure with slugs.
